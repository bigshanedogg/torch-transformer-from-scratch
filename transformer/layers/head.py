...
class PolyEncoderHead(nn.modules.Module):
    ...
    def forward(self, context_embed: Tensor, candidate_embed: Tensor = None) -> Tensor:
        # context_embed: (context_batch_size, m, d_model)
        # candidate_embed: (candidate_batch_size, d_model)
        context_batch_size = context_embed.shape[0]
        ...
        context_embed, context_attention_weights = dot_attention(query=candidate_embed_query, key=context_embed, value=context_embed)
        # output: (context_batch_size, candidate_batch_size)
        output = torch.sum(context_embed * candidate_embed, dim=-1)
