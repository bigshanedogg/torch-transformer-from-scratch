...
class PreprocessorInterface(PreprocessorAssertion):
    ...
    def sentence_to_ids(self, sentence: str, mask: bool = False, spm_tokenizer: SpmTokenizer = None, language: str = None) -> List[int]:
        ...
        token_ids = spm_tokenizer.tokens_to_ids(tokens=tokens, mask=mask)
        return token_ids

    def get_token_length(self, sentence: str, spm_tokenizer: SpmTokenizer, language: str) -> int:
        tokens = None
        if language == "eng":
        ...
        tokens = spm_tokenizer.tokens_to_ids(tokens=tokens, mask=False)
        length = len(tokens)
        return length
    ...
    def get_sep_token_ids(self, sep_tokens, num_segments, spm_tokenizer: SpmTokenizer):
        if sep_tokens is None:
            # sep_tokens = [["cls", "sep"], [None, "sep"], [None, "sep"]]
            # e.g.) <cls> ~ <sep> ~ <sep> ~ <sep>
            sep_token_ids = [[None, spm_tokenizer.special_token_dict["sep"]["id"]]] * (num_segments - 1)
            sep_token_ids.insert(0, [spm_tokenizer.special_token_dict["cls"]["id"], spm_tokenizer.special_token_dict["sep"]["id"]])
            ...
            for special_token_1, special_token_2 in sep_tokens:
                ...
                if special_token_1 is not None:
                    self.assert_isin_obj(element=special_token_1, obj=spm_tokenizer.special_token_dict.keys())
                    special_token_1_id = spm_tokenizer.special_token_dict[special_token_1]["id"]
                special_token_2_id = None
                if special_token_2 is not None:
                    self.assert_isin_obj(element=special_token_2, obj=spm_tokenizer.special_token_dict.keys())
                    special_token_2_id = spm_tokenizer.special_token_dict[special_token_2]["id"]
    ...
    def get_common_subtokens(self, sentence_a: str, sentence_b, spm_tokenizer: SpmTokenizer = None, language: str = None):
        subtokens_a = self.sentence_to_subtokens(sentence=sentence_a, spm_tokenizer=spm_tokenizer, language=language)
        subtokens_b = self.sentence_to_subtokens(sentence=sentence_b, spm_tokenizer=spm_tokenizer, language=language)
    ...
    def sentence_to_subtokens(self, sentence: str, spm_tokenizer: SpmTokenizer, language: str = None):
        tokens = None
        if language == "eng":
            tokens = self.mecab_tokenizer.tokenize_eng(sentence=sentence, return_pos=False)
        elif language == "kor":
            tokens = self.mecab_tokenizer.tokenize_kor(sentence=sentence, return_pos=False)
        subtokens = spm_tokenizer.tokens_to_pieces(tokens=tokens, mask=False)
        return subtokens
