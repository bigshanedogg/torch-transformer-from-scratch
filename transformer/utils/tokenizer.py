...
class SpmTokenizer(TokenizerAssertion):
    ...
    def _mask_token(self, token_id: int) -> int:
        ...
        if random_prob < self.keep_mask_ratio:
            ...
            while token_id in self.special_token_ids:
                token_id = np.random.choice(list(range(0, len(self.spm_tokens))))
    ...
    def tokens_to_pieces(self, tokens: List[str], mask: bool) -> List[str]:
        ...
        output = [self.spm_model.IdToPiece(token_id) for token_id in token_ids]
        return output

    def decode(self, ids: List[int]) -> str:
        self.assert_isinstance_list(ids, "ids")
        output = self.spm_model.Decode(ids)
        return output
    ...
    def _train_spm_model(self, input_data_path: str, vocab_size: int, path: str = None, spm_model_type: str = "bpe") -> None:
        ...
        if path is not None:
            self.save_spm_model(path=path, copy=False)
    ...
    def load_spm_model(self, path: str) -> None:
        ...
        special_token_dict_file = self.spm_model_path + "special_token_dict.pickle"
    ...
    def save_spm_model(self, path: str, copy: bool = False) -> None:
        if path != self.spm_model_path:
            if not path.endswith("/"): path = path + "/"
        ...
        if not copy:
            ...
            message = "saved spm_model: '{path}'".format(path=path)
