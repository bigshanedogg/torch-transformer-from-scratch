...
class TokenizerAssertion(Assertion):
    def assert_isloaded_spm_model(self):
...
class ModelAssertion(Assertion):
    ...
    def assert_isin_aggregation_methods(self, model):
...
class ServiceAssertion(Assertion):
    ...
    def assert_isloaded_spm_tokenizer(self):
        assert_message = "SpmTokenizer has not been loaded"
        if self.spm_tokenizer is None:
            logging.error(assert_message)
            raise AssertionError(assert_message)

    def assert_isloaded_src_spm_tokenizer(self):
        assert_message = "SrcSpmTokenizer has not been loaded"
        if self.src_spm_tokenizer is None:
            logging.error(assert_message)
            raise AssertionError(assert_message)

    def assert_isloaded_tgt_spm_tokenizer(self):
        assert_message = "TgtSpmTokenizer has not been loaded"
        if self.tgt_spm_tokenizer is None:
            logging.error(assert_message)
            raise AssertionError(assert_message)
