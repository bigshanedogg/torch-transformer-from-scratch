...
def main():
    ...
    trainer = GeneratorFinetuningTransformerTrainer(temp_dir=train_config["temp_save_path"])
    ...
    preprocessor = GeneratorFinetuningPreprocessor(src_language=data_config["src_language"], tgt_language=data_config["tgt_language"],
    ...
    ddp_params = GeneratorFinetuningTransformerTrainer.get_ddp_params(**ddp_config)
    train_params = GeneratorFinetuningTransformerTrainer.get_train_params(**train_config)
    ...
    data_params = GeneratorFinetuningTransformerTrainer.get_data_params(**data_config)
    ...
    if "ul" in criterion_config and criterion_config["ul"] > 0.0:
        ...
        train_dataset = DatasetFromDir(data_dir=data_config["train_data_dir"], batch_size=train_config["batch_size"], device="cpu", nprocs=nprocs,
                                       encoding=data_config["encoding"], extension=data_config["extension"])
